...
# Set the project directory to the new backend directory
PROJECT_DIR=$(dirname "$(realpath "$0")")
BACKEND_DIR="$PROJECT_DIR/backend"
LOG_FILE="$BACKEND_DIR/logs/install.log"
...
sudo chmod +x "$PROJECT_DIR/database/mysql_setup.sh" || exit_script
...
sudo "$PROJECT_DIR/database/mysql_setup.sh" || exit_script
...
# Suggest user to log out and back in for group changes to take effect
log "Please log out and log back in for Docker group changes to take effect, or run 'newgrp docker'."
...
#         sudo apt install -y python3 python3-pip || exit_script
#     else
#         log "Python3 is already installed."
#     fi
...
#     if ! command -v pip3 &> /dev/null; then
#         log "pip3 is not installed. Installing pip3..."
#         sudo apt update -y || exit_script
#         sudo apt install -y python3-pip || exit_script
#     else
#         log "pip3 is already installed."
#     fi
...
#     if ! command -v nginx &> /dev/null; then
#         log "Nginx is not installed. Installing Nginx..."
#         sudo apt install -y nginx || exit_script
#     else
#         log "Nginx is already installed."
#     fi
...
#         sudo add-apt-repository -y ppa:ondrej/php || exit_script
#         sudo apt update -y || exit_script
#         sudo apt install -y php8.1 php8.1-fpm php8.1-mysql || exit_script
#     else
#         PHP_VERSION=$(php -v | grep -oP '^PHP \K([0-9]+\.[0-9]+)')
#         if [[ "$PHP_VERSION" != "8.1" ]]; then
#             log "PHP version is not 8.1. Installing PHP 8.1 and PHP-FPM..."
...
# install_adminer() {
#     log "Installing Adminer..."
#     sudo wget https://www.adminer.org/latest.php -O /var/www/html/adminer.php || exit_script
#     sudo chown www-data:www-data /var/www/html/adminer.php || exit_script
#     sudo chmod 755 /var/www/html/adminer.php || exit_script
#     log "Adminer installed successfully."
# }
